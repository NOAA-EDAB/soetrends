## mode_select
## Function run the gam analysis and model selection that outputs 
## timeseries and model name
...
#test_ltsst <- ecodata::long_term_sst
#test<- mod_select(test_ltsst)

## Model run and selection 
mod_select <- function(indicator){
  ...
  rand <- data.frame(rep(1,length(indicator$Value))) %>%
    dplyr::rename("rand" = "rep.1..length.indicator.Value..")
  ind<- indicator %>% cbind(rand)
